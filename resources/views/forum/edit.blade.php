...
<head>
    ...
    <title>Irminsul Studio | Edit Thread</title>
        ...
        <div class="forum-create-card">
            ...
            <form method="POST" action="{{ route('forum.update', $thread) }}">
                ...
                <div class="forum-create-form-group">
                    ...
                    <input id="title" type="text" name="title" value="{{ old('title', $thread->title) }}" placeholder="Thread title" required>
                    @error('title')
                ...
                <div class="forum-create-form-group">
                    ...
                    <select id="game_id" name="game_id">
                        ...
                        @foreach($games as $game)
                            <option value="{{ $game->id }}" {{ old('game_id', $thread->game_id) == $game->id ? 'selected' : '' }}>{{ $game->name }}</option>
                        @endforeach
                    </select>
                    @error('game_id')
                        <span style="color: #f08a92; font-size: 0.8rem; margin-top: 6px; display: block;">{{ $message }}</span>
                ...
                <div class="forum-create-form-group">
                    ...
                    <select id="category" name="category">
                        ...
                        @foreach($categories as $key => $label)
                            <option value="{{ $key }}" {{ old('category', $thread->category) === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('category')
                        <span style="color: #f08a92; font-size: 0.8rem; margin-top: 6px; display: block;">{{ $message }}</span>
                ...
                <div class="forum-create-form-group">
                    ...
                    <textarea id="body" name="body" placeholder="Write your thread content..." required>{{ old('body', $thread->body) }}</textarea>
                    @error('body')
                        <span style="color: #f08a92; font-size: 0.8rem; margin-top: 6px; display: block;">{{ $message }}</span>
                    @enderror
                </div>

                <button type="submit" class="forum-create-submit">Update Thread</button>
            </form>
