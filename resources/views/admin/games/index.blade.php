...
<head>
    ...
    <title>Irminsul Studio | Manage Games</title>
    ...
    <div class="admin-section">
        ...
        <div class="mb-3">
            <a href="{{ route('admin.games.create') }}">
                <button class="admin-upload-btn"><i class="fas fa-plus"></i> Create Game</button>
            </a>
        </div>
                ...
                <tbody>
                    @forelse ($games as $game)
                        <tr>
                            <td>{{ $game->name }}</td>
                            <td>
                                <span class="badge bg-{{ $game->status === 'released' ? 'success' : ($game->status === 'beta' ? 'warning' : 'info') }}">
                                    {{ ucfirst($game->status) }}
                                </span>
                            </td>
                            <td>{{ $game->images->count() }}</td>
                            <td>
                            ...
                            <td>
                                <div class="admin-actions">
                                    ...
                                    <a href="{{ route('admin.games.images', $game->id) }}">
                                        <button class="admin-btn admin-btn-edit"><i class="fas fa-images"></i> Images</button>
                                    </a>
                                    <form action="{{ route('admin.games.destroy', $game->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this game?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center">No games yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
