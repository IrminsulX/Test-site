...
<head>
    ...
    <title>Irminsul Studio | Activity Log</title>
    <link rel="stylesheet" href="{{ asset('css/main.css') }}">
    <link rel="stylesheet" href="{{ asset('css/adminhomepage.css') }}">
    <style>
        .log-action { padding: 2px 10px; border-radius: 4px; font-size: 0.8rem; font-weight: bold; text-transform: uppercase; }
        .log-action.create { background: #4caf50; color: #fff; }
        .log-action.update { background: #ff9800; color: #fff; }
        .log-action.delete { background: #f44336; color: #fff; }
        ...
        <div class="admin-card">
            <div class="table-responsive"><table class="table table-dark table-striped table-hover">
                ...
                <tbody>
                    @forelse ($logs as $log)
                        <tr class="log-row">
                            <td style="white-space: nowrap;">{{ $log->created_at->format('M j, g:ia') }}</td>
                            <td>{{ $log->user?->name ?? 'System' }}</td>
                            <td><span class="log-action {{ $log->action }}">{{ $log->action }}</span></td>
                            <td>{{ $log->description }}</td>
                            <td style="font-family: monospace; font-size: 0.85rem;">{{ $log->ip }}</td>
                        </tr>
                    @empty
